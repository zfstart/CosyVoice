...
## 9. 生产建议：自动获取并校验 `spkId`

你可以在 CosyVoice FastAPI 服务里新增一个只读接口，直接返回当前模型可用的 speaker 列表（底层即 `list_available_spks()`）。

### 9.1 Python 侧新增 `/speakers`

在 `runtime/python/fastapi/server.py` 增加：
...
@app.get("/speakers")
async def speakers():
    return JSONResponse({"speakers": cosyvoice.list_available_spks()})
```

### 9.2 Spring Boot 启动时拉取并缓存
...
@Component
public class SpeakerRegistry implements ApplicationRunner {

    private final WebClient webClient;
    private volatile Set<String> speakers = Collections.emptySet();

    public SpeakerRegistry(WebClient cosyVoiceWebClient) {
        this.webClient = cosyVoiceWebClient;
    }
    ...
    public void run(ApplicationArguments args) {
        ...
        if (resp == null || !(resp.get("speakers") instanceof List<?> list)) {
        ...
        this.speakers = Collections.unmodifiableSet(tmp);
    }

    public boolean contains(String spkId) {
        return speakers.contains(spkId);
    }
...
public ResponseEntity<byte[]> sft(@RequestBody TtsRequest request) {
    ...
    if (!speakerRegistry.contains(spkId)) {
        return ResponseEntity.badRequest().build();
    }
...
这样你就可以避免“传了不存在的 `spkId` 导致推理异常”的问题。
